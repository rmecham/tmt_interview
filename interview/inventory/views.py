...
from interview.inventory.serializers import (InventoryLanguageSerializer,
                                             InventorySerializer,
...
class InventoryCreatedAfterView(APIView):
    """
    API View to list inventory items created after a date given in the URL.
    """
    queryset = Inventory.objects.all()
    serializer_class = InventorySerializer

    def get_queryset(self, **kwargs) -> QuerySet[Inventory]:
        """Filter the inventory items based on the date in the URL."""

        queryset = self.queryset.filter(
            created_at__gte=utc.localize(datetime(
                self.kwargs['year'],
                self.kwargs['month'],
                self.kwargs['day']
            ))
    ...
    def get(self, request: Request, *args, **kwargs) -> Response:
        """Standard response to GET requests."""
        serializer = self.serializer_class(self.get_queryset(), many=True)
